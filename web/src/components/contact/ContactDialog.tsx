...
export function ContactDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {open && process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY && (
        <Script
      ...
      <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto rounded-md p-6 sm:max-w-xl sm:p-8">
        <DialogHeader>
          <span className="text-xs font-semibold tracking-[0.2em] text-primary uppercase">
            Novo projeto
          </span>
          <DialogTitle className="text-3xl tracking-[-0.04em]">
            Conte-me sobre a sua ideia.
          </DialogTitle>
          <DialogDescription className="leading-relaxed">
            Envie um resumo do desafio. Se já tiver um escopo, pode anexá-lo à
            mensagem.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={submit} className="mt-3 grid gap-4 sm:grid-cols-2">
          ...
          <Textarea
            ...
            placeholder="Descreva o projeto, objetivo e prazo, se houver..."
            value={form.message}
            onChange={(event) =>
          ...
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-border px-4 py-3 text-left text-sm text-muted-foreground transition hover:border-primary/50 sm:col-span-2">
            ...
            <span className="min-w-0 truncate">
              {attachment
                ? attachment.name
                : "Anexar escopo — PDF, DOC, DOCX, TXT ou imagem (até 5 MB)"}
            </span>
            <input
              type="file"
          ...
          <Button
            disabled={submitting}
            className="rounded-full sm:col-span-2 sm:justify-self-end"
          >
            <Send />
